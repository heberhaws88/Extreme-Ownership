"""
Builds/rebuilds the Extreme Ownership: Wired RSS feed (feed.xml) from the
episode manifest.

Same design as the Daily show's feed_builder.py: keep a JSON manifest of
every published episode and regenerate the entire feed.xml from it on every
run, rather than mutating existing XML. This file is self-contained to the
extreme-ownership-wired/ folder and never touches Daily's data, feed, or
...
<rss version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:atom="http://www.w3.org/2005/Atom">
  <channel>
    ...
    <itunes:category text="{escape(config.get('itunes_category', 'Business'))}">
      <itunes:category text="{escape(config.get('itunes_subcategory', 'Management'))}" />
    </itunes:category>
{items_xml}
  </channel>
</rss>
